...
import { Calendar, Users, CreditCard, Clock, SlidersHorizontal, CheckCircle2 } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 my-6">
      ...
      <div className="relative group p-6 rounded-2xl bg-white dark:bg-[#0D1B2A] border border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md dark:shadow-slate-900/50 transition-all duration-200">
        ...
        <div className="mt-2 flex items-baseline gap-2">
          <span className="text-3xl font-extrabold text-slate-900 dark:text-white">{quotation.travelers}</span>
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">{quotation.travelers === 1 ? 'Pasajero total' : 'Pasajeros totales'}</span>
        </div>
        {formData && (formData.adultsCount || formData.childrenCount || formData.babiesCount) ? (
      ...
      <div className="relative group p-6 rounded-2xl bg-gradient-to-br from-white via-white to-teal-50/30 dark:from-[#0D1B2A] dark:via-[#0D1B2A] dark:to-[#0F2A4A]/40 border border-[#00A896]/30 dark:border-[#00A896]/40 shadow-sm hover:shadow-md transition-all duration-200">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-[#00A896]/15 text-[#00A896] border border-[#00A896]/20">Total Final</span>
        </div>
