...
export function BrokerCard({ broker, quotationId }) {
  ...
  const agencyLabel = broker.agency_name || 'Karabu Viajes';
  const roleLabel = broker.role === 'super_admin' ? 'Administrador' 
    : broker.role === 'admin' ? 'Asesor' 
    : broker.role === 'advisor' ? 'Asesor' 
    : broker.role || 'Asesor';
  // If no role set (legacy), default to "Asesor"
  const displayRole = roleLabel;
  const avatarUrl = broker.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(broker.name || 'AK')}&background=0F2A4A&color=fff&size=200`;

  return (
    <div className="w-full mt-10 mb-8 p-6 sm:p-8 rounded-2xl sm:rounded-3xl bg-white dark:bg-[#0D1B2A] border border-slate-200/80 dark:border-slate-800 shadow-sm relative">
