  ...
  const timeline = useMemo(() => {
    ...
    (data.quotations || []).forEach(q => {
      events.push({
        ...
        color: q.status === "aceptada" ? "bg-green-500" : q.status === "rechazada" ? "bg-red-500" : "bg-yellow-500",
        icon: FileText
      });
    });
    ...
    (data.reservations || []).forEach(r => {
      events.push({
        ...
        color: r.status === "pagada" ? "bg-green-500" : r.status === "cancelada" ? "bg-gray-500" : "bg-blue-500",
        icon: CalendarCheck
      });
    });
    ...
    <div data-testid="client-detail-page" className="space-y-6">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[16px] border border-gray-200 p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.05),_0_1px_2px_-1px_rgba(0,0,0,0.05)]">
            <div className="flex flex-col items-center text-center pb-6 border-b border-gray-100">
              ...
              <span className="text-xs text-gray-500 mt-0.5">Cliente desde: {formatDate(client.created_at)}</span>
            </div>
            ...
            <div className="py-4 space-y-3.5 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs text-gray-400 block font-medium">Correo Electrónico</span>
                  <a href={`mailto:${client.email}`} className="text-gray-700 hover:text-blue-600 break-all font-medium">{client.email}</a>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Phone className="h-4 w-4 text-gray-400 mt-0.5" />
                <div className="flex-1">
                  <span className="text-xs text-gray-400 block font-medium">Teléfono</span>
                  <a href={`tel:${client.phone}`} className="text-gray-700 hover:text-blue-600 font-medium">{client.phone}</a>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <FileText className="h-4 w-4 text-gray-400 mt-0.5" />
                <div className="flex-1">
                  <span className="text-xs text-gray-400 block font-medium">Documento de Identidad</span>
                  <span className="text-gray-700 font-medium">{client.document_id || "No registrado"}</span>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <MapPin className="h-4 w-4 text-gray-400 mt-0.5" />
                <div className="flex-1">
                  <span className="text-xs text-gray-400 block font-medium">Dirección</span>
                  <span className="text-gray-700 font-medium">{client.address || "No registrada"}</span>
                </div>
              </div>
            </div>

            <div className="pt-4 border-t border-gray-100 flex items-center justify-around text-center">
              <div>
                <span className="text-lg font-bold text-gray-900 block">{reservations.length}</span>
                <span className="text-xs text-gray-400 font-medium">Reservas</span>
              </div>
              <div className="border-r border-gray-200 h-8"></div>
              <div>
                <span className="text-lg font-bold text-gray-900 block">{quotations.length}</span>
                <span className="text-xs text-gray-400 font-medium">Cotizaciones</span>
              </div>
              <div className="border-r border-gray-200 h-8"></div>
              <div>
                ...
                <span className="text-xs text-gray-400 font-medium">Invertido</span>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="col-span-12 lg:col-span-8 space-y-6">
          <div className="bg-white rounded-[16px] border border-gray-200 shadow-[0_1px_3px_0_rgba(0,0,0,0.05),_0_1px_2px_-1px_rgba(0,0,0,0.05)] overflow-hidden">
            {/* Tabs Header */}
                    ...
                    <div className="overflow-x-auto border border-gray-200 rounded-[10px]">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Destino</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Fecha Salida</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Monto Total</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Estado</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Ver</th>
                          </tr>
                        </thead>
                        <tbody>
                          {reservations.map(r => (
                            <tr key={r.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-gray-600">{formatDate(r.departure_date)}</td>
                              <td className="px-4 py-3 text-gray-900">{formatCurrency(r.total_amount, r.currency)}</td>
                    ...
                    <div className="overflow-x-auto border border-gray-200 rounded-[10px]">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 border-b border-gray-200">
                          <tr>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Destino</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Fecha Viaje</th>
                            <th className="px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Viajeros</th>
                        ...
                        <tbody>
                          {quotations.map(q => (
                            <tr key={q.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                              <td className="px-4 py-3 font-medium text-gray-900">{q.destination}</td>
                              <td className="px-4 py-3 text-gray-600">{q.travel_date ? formatDate(q.travel_date) : "Sin definir"}</td>
                              <td className="px-4 py-3 text-gray-700">{q.travelers}</td>
                              <td className="px-4 py-3 text-gray-900">{formatCurrency(q.amount, q.currency)}</td>
                        ...
                        <tbody>
                          {payments.map(p => (
                            <tr key={p.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-gray-900 font-semibold">{formatCurrency(p.amount, p.currency)}</td>
              ...
              {tab === "notas" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-gray-400">Las notas son solo visibles para asesores y administradores.</p>
                  </div>
                  <textarea
                    rows={8}
                    value={notesText}
                    ...
                    className="w-full rounded-[10px] border border-gray-300 px-3.5 py-3 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-shadow"
                  />
              ...
              {tab === "historial" && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-400">Total de eventos: {timeline.length}</span>
                  </div>
                    ...
                    {timeline.map((event, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative flex items-start gap-4">
                          ...
                          {/* Event details */}
                          <div className="flex-1 bg-gray-50/50 hover:bg-gray-50 border border-gray-100 rounded-[12px] p-4 transition-colors">
                            ...
                            <p className="text-xs text-gray-600 leading-normal">{event.desc}</p>
                          </div>
                        </div>
                      );
                    })}
