...
import { Plus, Search, Loader2, Pencil, Trash2, UserCog, X, ShieldCheck } from "lucide-react";
...
export default function Users() {
  ...
  const openEdit = (u) => {
    setEditingId(u.id);
    setForm({ name: u.name, email: u.email, role: u.role, status: u.status || "activo", password: "" });
    setModalOpen(true);
  };
          ...
          <form onSubmit={save} className="mt-16 bg-white dark:bg-gray-900 rounded-[16px] shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <Field label={editingId ? "Nueva contraseña (dejar vacío para mantener)" : "Contraseña"} required={!editingId}>
                <input data-testid="user-password" type="password" required={!editingId} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={inputCls} minLength={6} />
              </Field>
            </div>
