...
import { 
  Printer, ArrowLeft, User, Calendar, CreditCard, Mail, Phone, MapPin, 
  FileText, ShieldCheck, Plus, Trash2, Edit3, DollarSign
} from "lucide-react";
...
export default function InvoiceView() {
  ...
  const [client, setClient] = useState({
    name: "Cliente Karabu", email: "cliente@example.com", phone: "+1 (809) 000-0000",
    document_id: "402-2849102-3", address: "Santo Domingo, R.D."
  });
  ...
  const [items, setItems] = useState([
    { id: "1", description: "Vuelos Internacionales I/V", quantity: 2, unitPrice: 650 },
    { id: "2", description: "Alojamiento Resort All-Inclusive (7 noches)", quantity: 1, unitPrice: 1800 },
    { id: "3", description: "Traslados Privados Aeropuerto - Hotel", quantity: 1, unitPrice: 150 },
    { id: "4", description: "Seguro de Viaje y Asistencia Médica", quantity: 2, unitPrice: 75 },
  ]);
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (qData) {
          ...
          if (cliData) {
            setClient({
              name: `${cliData.first_name || ''} ${cliData.last_name || ''}`.trim() || "Cliente Karabu",
              email: cliData.email || "cliente@example.com",
              phone: cliData.phone || "+1 (809) 000-0000",
              document_id: cliData.document_id || "402-2849102-3",
              address: cliData.address || "Santo Domingo, R.D."
            });
          }
          if (Array.isArray(qData.services) && qData.services.length > 0) {
            ...
            const hotelPart = Math.round(totalAmt * 0.55);
            const flightPart = Math.round(totalAmt * 0.30);
            const rest = totalAmt - (hotelPart + flightPart);
            setItems([
              { id: "1", description: `Hospedaje en ${qData.destination || 'Destino'} (${qData.assigned_hotel || 'Resort'})`, quantity: 1, unitPrice: hotelPart },
              { id: "2", description: `Vuelos para ${qData.travelers || 2} pasajeros`, quantity: 1, unitPrice: flightPart },
              { id: "3", description: "Traslados y Seguro de Viaje", quantity: 1, unitPrice: rest },
            ]);
          }
        }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-zinc-950 text-slate-800 dark:text-zinc-100 antialiased py-6 px-3 sm:px-6">
      <div className="max-w-4xl mx-auto mb-6 print:hidden flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-200/80 dark:border-zinc-800 shadow-md">
        <button onClick={() => navigate(-1)} className="h-9 px-3 text-xs font-semibold rounded-xl border border-slate-200 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-800 flex items-center gap-1.5"><ArrowLeft className="w-4 h-4" /> Volver</button>
        ...
        <div className="p-8 sm:p-12 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-start justify-between gap-6 pb-6 border-b border-slate-200 dark:border-zinc-800">
            <div className="space-y-3 max-w-md">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#0F2A4A] text-white flex items-center justify-center font-black text-xl shadow-lg"><span className="text-[#0D9387]">K</span></div>
                <div><h1 className="text-2xl font-black text-[#0F2A4A] dark:text-zinc-100">KARABU</h1><span className="text-xs font-bold text-[#0D9387] uppercase tracking-widest">Viajes & Visas</span></div>
              </div>
          ...
          <div className="bg-slate-50/80 dark:bg-zinc-800/50 p-5 rounded-2xl border border-slate-200/80 dark:border-zinc-700/80">
            ...
            <div className="space-y-1 text-xs">
              ...
              <p>Cédula/Documento: <span className="font-bold text-slate-800 dark:text-zinc-200">{client.document_id}</span></p>
              <p className="text-slate-500">{client.address}</p>
              <p className="flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-[#0D9387]" />{client.email} • <Phone className="w-3.5 h-3.5 text-[#0D9387]" />{client.phone}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/60 border border-slate-200/80 dark:border-zinc-700/80 space-y-3">
              ...
              <div className="p-3 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 space-y-1">
                ...
                <p className="font-bold text-slate-800 dark:text-zinc-200">Banco Popular Dominicano</p>
                <p className="font-mono text-xs">Cta Cte #: <strong>812-49102-3</strong></p>
                <p className="text-[11px]">Titular: Karabu Viajes & Visas S.R.L. • RNC: 1-32-84920-1</p>
              </div>
            </div>
