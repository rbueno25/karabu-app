      ...
      <div className="bg-white dark:bg-[#0F2444] rounded-[16px] border border-gray-200 dark:border-[#1A3356] shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-[#1A3356] flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input data-testid="destinations-search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por destino o país…" className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 dark:border-[#1A3356] bg-white dark:bg-[#0F2444] text-gray-900 dark:text-gray-100 rounded-[10px] focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none" />
          </div>
          <select data-testid="destinations-status-filter" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="text-sm border border-gray-200 dark:border-[#1A3356] rounded-[10px] px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none bg-white dark:bg-[#0F2444] text-gray-900 dark:text-gray-100">
        ...
        {loading ? (
          ...
          <>
            <table className="w-full text-sm" data-testid="destinations-table">
              ...
              <tbody>
                {paginated.map((d) => (
                  <tr key={d.id} className="border-b border-gray-100 dark:border-[#1A3356] last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800">
                    <td className="px-6 py-3">
                      {d.image_url ? (
                        <img src={d.image_url} alt={d.name} className="h-10 w-16 object-cover rounded-[8px] border border-gray-200 dark:border-[#1A3356]" />
                      ) : (
                        <div className="h-10 w-16 rounded-[8px] bg-gray-100 dark:bg-[#132D52] border border-gray-200 dark:border-[#1A3356] flex items-center justify-center text-gray-400 dark:text-gray-400">
...
function Modal({ onClose, title, onSubmit, saving, children, testId, saveTestId }) {
  return (
    <div data-testid={testId} className="fixed inset-0 z-50 flex items-start justify-center bg-gray-900/50 backdrop-blur-sm p-4 overflow-y-auto">
      <form onSubmit={onSubmit} className="mt-16 bg-white dark:bg-[#0F2444] rounded-[16px] shadow-xl border border-gray-200 dark:border-[#1A3356] w-full max-w-lg p-6">
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-[#1A3356] rounded-[10px] hover:bg-gray-50 dark:hover:bg-gray-800">Cancelar</button>
