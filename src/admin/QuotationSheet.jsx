...
export default function QuotationSheet() {
  ...
  const loadQuotation = async () => {
    ...
    try {
      ...
      setForm({
        ...
        destination: res.data.quotation.destination,
        travel_date: (res.data.quotation.travel_date || "").slice(0, 10),
        return_date: (res.data.quotation.return_date || "").slice(0, 10),
        travelers: res.data.quotation.travelers || 1,
        amount: res.data.quotation.amount || 0,
        currency: res.data.quotation.currency || "USD",
        notes: res.data.quotation.notes || "",
        status: res.data.quotation.status || "borrador",
        sent_via: res.data.quotation.sent_via || "",
  ...
  return (
    <div data-testid="quotation-sheet-page" className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500">ID Cotización: {quotation.id}</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <form onSubmit={handleSave} className="col-span-12 lg:col-span-8 bg-white border border-gray-200 rounded-[16px] shadow-[0_1px_3px_0_rgba(0,0,0,0.05),_0_1px_2px_-1px_rgba(0,0,0,0.05)] overflow-hidden">
          {/* Header del documento */}
          <div className="p-8 border-b border-gray-100 bg-gray-50/40 flex justify-between items-start flex-wrap gap-4">
            <div>
              <div className="text-xl font-bold text-gray-950 flex items-center gap-2">
                <FileText className="h-5 w-5 text-blue-600" />
                <span>KARABU VIAJES</span>
              </div>
              <p className="text-xs text-gray-500 mt-1">Propuesta y Presupuesto de Servicios Turísticos</p>
            </div>
            <div className="text-right text-xs text-gray-500">
              <p className="font-semibold text-gray-800">Fecha de Creación:</p>
              <p>{formatDate(quotation.created_at)}</p>
              {quotation.sent_at && (
                <div className="mt-1">
                  <p className="font-semibold text-gray-800">Fecha de Envío:</p>
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50/50 rounded-[12px] border border-gray-100">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">Para el Cliente</span>
                <div className="space-y-1 text-sm">
                  <h4 className="font-bold text-gray-900">{client.first_name} {client.last_name}</h4>
                  <p className="text-gray-600 flex items-center gap-1.5"><Mail className="h-3.5 w-3.5 text-gray-400" /> {client.email}</p>
                  <p className="text-gray-600 flex items-center gap-1.5"><Phone className="h-3.5 w-3.5 text-gray-400" /> {client.phone}</p>
                </div>
              </div>
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">Asesor Asignado (Broker)</span>
                <div className="space-y-1 text-sm">
                  <h4 className="font-bold text-gray-900">{broker.name}</h4>
                  <p className="text-gray-600 flex items-center gap-1.5"><Mail className="h-3.5 w-3.5 text-gray-400" /> {broker.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-900 border-b border-gray-100 pb-2">Destino y Fechas</h3>
            ...
            <div className="space-y-4 pt-2">
              <h3 className="text-sm font-semibold text-gray-900 border-b border-gray-100 pb-2">Precio de la Propuesta</h3>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50/40 flex items-center justify-end gap-2">
            <button
              type="submit"
              disabled={saving}
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[16px] border border-gray-200 p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.05)]">
            <h3 className="text-sm font-semibold text-gray-900 border-b border-gray-100 pb-3 mb-4">Información del Sistema</h3>
            <div className="space-y-3.5 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-700 font-medium">{formatDate(quotation.updated_at)}</span>
              </div>
              {quotation.sent_via && (
                <div className="pt-2.5 border-t border-gray-100 space-y-2">
                  <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block">Información de Envío</span>
                  <div className="flex justify-between">
                    <span className="text-gray-400 font-medium">Enviado vía:</span>
                    <span className="text-gray-700 font-semibold capitalize">{quotation.sent_via}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400 font-medium">Fecha:</span>
                    <span className="text-gray-700 font-medium">{formatDate(quotation.sent_at)}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {["enviada", "aceptada", "rechazada"].includes(form.status) && (
              <div className="mt-6 pt-4 border-t border-gray-100 space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block">Enlace de Propuesta</span>
                <p className="text-xs text-gray-500 leading-normal">Esta cotización tiene una propuesta web interactiva activa para el cliente:</p>
                <div className="flex items-center gap-1">
                  <input readOnly value={clientLink} className="flex-1 bg-gray-50 border border-gray-200 rounded-[8px] px-2 py-1.5 text-xs text-gray-600 font-mono focus:outline-none" />
                  <button onClick={copyClientLink} className="h-8 w-8 bg-gray-100 hover:bg-gray-200 rounded-[8px] flex items-center justify-center text-gray-600 shrink-0 transition-colors"><Clipboard className="h-4 w-4" /></button>
                </div>
                <a href={clientLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-700 text-xs font-semibold hover:underline">
      ...
      {sendModalOpen && (
        <div data-testid="send-modal" className="fixed inset-0 z-50 flex items-start justify-center bg-gray-900/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="mt-16 bg-white rounded-[16px] shadow-xl border border-gray-200 w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-gray-900">Enviar Cotización</h3>
              <button type="button" onClick={() => setSendModalOpen(false)} className="h-8 w-8 rounded-[8px] hover:bg-gray-100 flex items-center justify-center">
                <X className="h-4 w-4 text-gray-500" />
              </button>
            </div>
            
            <p className="text-sm text-gray-500 mb-5">Elige el canal por el cual deseas enviarle la propuesta interactiva al cliente.</p>
      ...
      {successModalOpen && (
        <div data-testid="success-modal" className="fixed inset-0 z-50 flex items-start justify-center bg-gray-900/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="mt-16 bg-white rounded-[16px] shadow-xl border border-gray-200 w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-base font-bold text-gray-900">Propuesta Generada Correctamente</h3>
              <button type="button" onClick={() => setSuccessModalOpen(false)} className="h-8 w-8 rounded-[8px] hover:bg-gray-100 flex items-center justify-center">
                <X className="h-4 w-4 text-gray-500" />
              </button>
            </div>
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block">Enlace Público de la Propuesta</span>
              <div className="flex items-center gap-1.5">
                <input readOnly value={clientLink} className="flex-1 bg-gray-50 border border-gray-200 rounded-[8px] px-3 py-2 text-xs text-gray-600 font-mono focus:outline-none" />
                <button onClick={copyClientLink} className="h-9 w-9 bg-gray-100 hover:bg-gray-250 rounded-[8px] flex items-center justify-center text-gray-600 shrink-0 transition-colors"><Clipboard className="h-4 w-4" /></button>
              </div>
            </div>
            ...
            {sendingPlatform === "whatsapp" ? (
              <div className="pt-2">
                ...
                <p className="text-[10px] text-gray-400 text-center mt-1.5">Se abrirá WhatsApp Web con un mensaje y enlace pre-llenados para tu cliente.</p>
              </div>
            ) : (
              <div className="pt-2 bg-gray-50 p-4 rounded-[10px] border border-gray-150">
                <span className="text-xs font-bold text-gray-600 block mb-1">Correo Preparado</span>
